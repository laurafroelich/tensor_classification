function [Us, outputs, Ys] = ManTDA_normsratio(Xs, classes, varargin)
% [Us, outsmode1, outsmode2, outerwhile, Ys] = ManTDA_normsratio(Xs, classes, varargin)
...
if ~isempty(varargin)
    ...
    if length(varargin)>=7 && ~isempty(varargin{7})
        if ismember(varargin{7}, {'ManOpt', 'bo13', 'wen12'})
            ...
            warning('ManTDA_normsratio.m: chosen manifold optimiser is not an option. Using ManOpt.')
        end
    end
end
...
switch optmeth
    ...
    case 'bo13'
        [U, outs] = OptStiManAFBB_myvariant(U,...
            @tensorsldaobj_matrixdata_normsratio, opts, cmean_m_xmeans, xi_m_cmeans,...
            nis, K1, K2, Rw, Rb);
        fvals = outs.FArray;
